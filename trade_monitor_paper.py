...
def init_db_tables(conn, db_type):
  """Ensures portfolio and trades tables exist for both MySQL and SQLite."""
  cursor = conn.cursor()
  ...
  if not cursor.fetchone():
    cursor.execute(
        f'INSERT INTO portfolio (id, total_capital, available_capital,'
        ' frozen_margin) VALUES'
        f' ({placeholder}, 100.0, 100.0, 0.0)',
        (1,),
    )
...
def get_db_connection():
  ...
  db_pass = os.environ.get(
      'PASS_DB_2', os.environ.get('MYSQL_PASSWORD', '')
  ).strip()
...
# =========================================================
# 📋 GENERATE & SEND CLEAN NTFY REPORT
# =========================================================
def generate_and_send_report():
  ...
  cursor.execute(
      f'SELECT status, pnl FROM trades WHERE status != {ph}', ('ACTIVE',)
  )
